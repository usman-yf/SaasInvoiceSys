        ...
        if (password_verify($password, $row['password'])) {
            if ($row['email_verified'] == 0) {
                ...
                if ($enable_2fa === 'yes') {
                    ...
                    header("Location: /inv/auth/2fa.php");
                    exit();
                } else {
                    // Normal Login Flow
                ...
                <form action="" method="post">
                    <div class="mb-5">
                        ...
                        <div class="relative">
                            ...
                            <input type="email" name="email" class="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block transition-colors outline-none shadow-sm" value="<?= isset($_COOKIE['remember_email']) ? htmlspecialchars($_COOKIE['remember_email']) : '' ?>" required <?= isset($_COOKIE['remember_email']) ? '' : 'autofocus' ?>>
                        </div>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="relative">
                            ...
                            ?>
                            <input type="password" id="passwordInput" name="password" class="w-full pl-10 pr-10 py-3 bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block transition-colors outline-none" value="<?= htmlspecialchars($remembered_password) ?>" required>
                    ...
                    <div class="mb-6 flex items-center justify-between">
                        <label class="flex items-center cursor-pointer group">
                            <div class="relative flex items-center justify-center w-5 h-5 mr-3">
                                <input type="checkbox" name="remember_me" class="peer sr-only" <?= isset($_COOKIE['remember_email']) ? 'checked' : '' ?>>
                                <div class="w-5 h-5 bg-gray-50 border-2 border-gray-300 rounded-md peer-checked:bg-brand-600 peer-checked:border-brand-600 transition-all duration-200 shadow-sm"></div>
                                <i data-lucide="check" class="absolute w-3.5 h-3.5 text-white opacity-0 peer-checked:opacity-100 transition-opacity duration-200 pointer-events-none"></i>
                            </div>
                    ...
                    <button type="submit" id="loginBtn" class="w-full bg-brand-600 hover:bg-brand-700 text-white font-medium py-3 px-4 rounded-xl shadow-sm hover:shadow transition-all flex items-center justify-center group">
