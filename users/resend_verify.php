...
if ($id > 0) {
    ...
    if ($row = mysqli_fetch_assoc($res)) {
        if ($row['email_verified'] == 1) {
            redirect('/inv/users/list.php?msg=' . urlencode("User '{$row['full_name']}' is already verified."));
        } else {
            ...
            redirect('/inv/users/list.php?msg=' . urlencode("Verification email resent to '{$row['full_name']}'."));
        }
    }
}
