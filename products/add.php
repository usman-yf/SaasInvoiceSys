...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if(empty($name) || $price < 0) {
        $error = "Name is required and price must be valid.";
    } else {
        $sql = "INSERT INTO products (name, sku, price, tax) VALUES ('$name', '$sku', $price, $tax)";
        if (mysqli_query($conn, $sql)) {
            if (isset($_SESSION['user_id'])) {
                logActivity($conn, $_SESSION['user_id'], 'Product Created', "Created product {$name}");
            }
            redirect(BASE_URL . '/products/list.php?msg=' . urlencode("Product '{$name}' successfully created."));
        } else {
...
<div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between space-y-4 sm:space-y-0">
    <div>
        ...
        <p class="text-gray-500 mt-1">Create a new product or service</p>
    </div>
        ...
        <div class="p-6 sm:p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="name" class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors outline-none shadow-sm" required>
                </div>
                <div>
                    ...
                    <input type="text" name="sku" class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors outline-none shadow-sm font-mono placeholder-gray-400" placeholder="e.g. ITEM-001">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.01" name="price" class="w-full pl-9 bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors outline-none shadow-sm" required>
                    </div>
                </div>
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.01" name="tax" class="w-full pr-9 bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors outline-none shadow-sm" value="0.00">
                    </div>
                </div>
            </div>
        </div>
