    ...
    if (empty($name) || empty($email) || empty($phone)) {
        ...
        if(mysqli_num_rows($check) > 0) {
            ...
            $sql = "UPDATE customers SET 
                    ...
                    vat_number='$vat_number', cr_number='$cr_number', building_no='$building_no', 
                    street='$street', district='$district', city='$city', postal_code='$postal_code', 
                    country='$country', additional_no='$additional_no', neighborhood='$neighborhood' 
                    WHERE id=$id";
        ...
        <div class="p-6 sm:p-8 border-t border-gray-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="vat_number" class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors outline-none shadow-sm" value="<?= htmlspecialchars($row['vat_number'] ?? '') ?>" placeholder="15 Digit VAT">
                </div>
                <div>
                    <label class="flex items-center text-sm font-medium text-gray-700 mb-2">CR Number</label>
                    <input type="text" name="cr_number" class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors outline-none shadow-sm" value="<?= htmlspecialchars($row['cr_number'] ?? '') ?>" placeholder="Commercial Registration">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div>
                    ...
                    <input type="text" name="building_no" class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors outline-none shadow-sm" value="<?= htmlspecialchars($row['building_no'] ?? '') ?>">
                </div>
                <div>
                    <label class="flex items-center text-sm font-medium text-gray-700 mb-2">Street</label>
                    <input type="text" name="street" class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors outline-none shadow-sm" value="<?= htmlspecialchars($row['street'] ?? '') ?>">
                </div>
                <div>
                    <label class="flex items-center text-sm font-medium text-gray-700 mb-2">Neighborhood</label>
                    <input type="text" name="neighborhood" class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors outline-none shadow-sm" value="<?= htmlspecialchars($row['neighborhood'] ?? '') ?>">
                </div>
                <div>
                    <label class="flex items-center text-sm font-medium text-gray-700 mb-2">District</label>
                    <input type="text" name="district" class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors outline-none shadow-sm" value="<?= htmlspecialchars($row['district'] ?? '') ?>">
                </div>
                <div>
                    <label class="flex items-center text-sm font-medium text-gray-700 mb-2">City</label>
                    <input type="text" name="city" class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors outline-none shadow-sm" value="<?= htmlspecialchars($row['city'] ?? '') ?>">
                </div>
                <div>
                    <label class="flex items-center text-sm font-medium text-gray-700 mb-2">Postal Code</label>
                    <input type="text" name="postal_code" class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors outline-none shadow-sm" value="<?= htmlspecialchars($row['postal_code'] ?? '') ?>">
                </div>
                <div>
                    <label class="flex items-center text-sm font-medium text-gray-700 mb-2">Country Code</label>
                    <input type="text" name="country" class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors outline-none shadow-sm" value="<?= htmlspecialchars($row['country'] ?? 'SA') ?>">
                </div>
                <div>
                    <label class="flex items-center text-sm font-medium text-gray-700 mb-2">Additional No</label>
                    <input type="text" name="additional_no" class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors outline-none shadow-sm" value="<?= htmlspecialchars($row['additional_no'] ?? '') ?>">
                </div>
            </div>
        </div>
